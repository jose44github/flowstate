...
use gpui::{
    App, AvailableSpace, Background, Bounds, Context, CursorStyle, Element, ElementId, Entity,
    FocusHandle, Focusable, FontStyle, FontWeight, GlobalElementId, Hsla, InspectorElementId,
    IntoElement, LayoutId, MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels,
    Point, Render, ShapedLine, SharedString, Size, Style, TextRun as GpuiTextRun, Window, black,
    div, fill, font, hsla, point, prelude::*, px, rgb, size,
};
...
impl Default for DocumentTheme {
    fn default() -> Self {
        Self {
            default_font_family: "Calibri".into(),
...
impl RichTextEditor {
    ...
    pub fn apply_run_style_to_selection(&mut self, style: RunStyle, cx: &mut Context<Self>) {
        ...
        for paragraph_ix in range.start.paragraph..=range.end.paragraph {
            ...
            apply_style_to_paragraph_range(
                &mut self.document.paragraphs[paragraph_ix],
                start..end,
                style,
            );
        }
    ...
    #[allow(dead_code)]
    pub fn set_paragraph_style_for_selection(
        &mut self,
...
impl Element for WordDocumentElement {
    ...
    fn request_layout(
        ...
        request_word_layout(
            self.editor.read(cx).document.clone(),
            self.layout.clone(),
            window,
        )
    }
...
fn request_word_layout(
    document: Document,
    layout_cell: WordElementLayout,
    window: &mut Window,
) -> (LayoutId, ()) {
    let layout_id =
        window.request_measured_layout(Style::default(), move |known, available, window, cx| {
            let width = known
                .width
...
fn run_format(
    document: &Document,
    paragraph: EffectiveParagraphFormat,
    styles: RunStyles,
) -> EffectiveRunFormat {
    let theme = &document.theme;
...
fn shape_line(
    ...
    for fragment in fragments {
        ...
        let format = run_format(document, p_format, fragment.styles);
...
fn shape_fragment(window: &mut Window, text: &str, format: EffectiveRunFormat) -> ShapedLine {
    ...
    run_font.weight = if format.bold {
        FontWeight::BOLD
    } else {
        FontWeight::NORMAL
    };
    run_font.style = if format.italic {
        FontStyle::Italic
    } else {
        FontStyle::Normal
    };
...
fn rects_for_line(document: &Document, line: &LaidOutLine) -> Vec<RunRect> {
    ...
    for segment in &line.segments {
        ...
        let paint_box = Bounds::new(
            ...
            size(
                (segment.width + highlight_pad_left + highlight_pad_right).max(px(1.0)),
                paint_height,
            ),
        );
        ...
        if segment.format.border_width > px(0.0) {
            let box_bounds = Bounds::new(
                point(
                    segment.x - document.theme.box_padding_left,
                    text_top - document.theme.box_padding_top,
                ),
                size(
                    (segment.width
                        + document.theme.box_padding_left
                    ...
                    (text_bottom - text_top
                        + document.theme.box_padding_top
                        + document.theme.box_padding_bottom)
                        .max(px(1.0)),
                ),
            );
            push_box_rules(
        ...
        match segment.format.underline {
            ...
            UnderlineKind::Single => {
                let (offset, thickness) =
                    single_underline_metrics_for_segment(segment, document, cx);
                underlines.push(Decoration {
                    bounds: Bounds::new(
                        point(segment.x, baseline + offset),
            ...
            UnderlineKind::Double => {
                ...
                underlines.push(Decoration {
                    bounds: Bounds::new(
                        point(
                            segment.x,
                            y + thickness + document.theme.double_underline_gap,
                        ),
                        size(segment.width.max(px(1.0)), thickness),
                    ),
                    color: document.theme.default_text_color,
...
fn single_underline_metrics_for_segment(
    ...
    let offset = regular_underscore_bounds(segment, cx)
        .map(|bounds| {
            ...
            (bounds.origin.y.abs() - bounds.size.height).max(px(0.0))
        })
...
fn double_underline_metrics_for_segment(document: &Document) -> (Pixels, Pixels) {
    (
        document.theme.double_underline_top_from_baseline,
        document.theme.underline_rule_thickness,
    )
}

fn regular_underscore_bounds(segment: &LaidOutSegment, cx: &mut App) -> Option<Bounds<Pixels>> {
    let mut underline_font = font(segment.format.font_family);
    ...
    underline_font.style = if segment.format.italic {
        FontStyle::Italic
    } else {
        FontStyle::Normal
    };
...
fn paint_layout(
    ...
    for paragraph in &layout.paragraphs {
        for border in &paragraph.borders {
            let border_bounds =
                snap_rule_bounds(border.bounds.shift(bounds.origin), border.snap, window);
    ...
    for paragraph in &layout.paragraphs {
        for line in &paragraph.lines {
            for rect in &line.rects {
                let rect_bounds = snap_rule_bounds(
                    rect.bounds.shift(bounds.origin + line.origin),
                    rect.snap,
                    window,
                );
        ...
        for line in &paragraph.lines {
            for underline in &line.underlines {
                ...
                if layout.snap_underline_rules_to_pixels {
                    underline_bounds =
                        snap_horizontal_rule_to_device_pixels(underline_bounds, window);
                }
...
fn snap_horizontal_rule_to_device_pixels(
...
fn snap_vertical_rule_to_device_pixels(
    mut bounds: Bounds<Pixels>,
    window: &Window,
) -> Bounds<Pixels> {
    let scale = window.scale_factor();
...
fn paint_selection(
    ...
    for paragraph in &layout.paragraphs {
        ...
        let start = if paragraph.index == range.start.paragraph {
            range.start.byte
        } else {
            0
        };
        ...
        for line in &paragraph.lines {
            ...
            window.paint_quad(fill(
                Bounds::new(
                    origin + line.origin + point(x1, px(0.0)),
                    size((x2 - x1).max(px(1.0)), line.line_height),
                ),
...
fn caret_bounds(
    ...
    Some(Bounds::new(
        origin + line.origin + point(x, px(0.0)),
        size(px(1.0), line.line_height),
    ))
}
    ...
    for segment in &line.segments {
        ...
        for run in &segment.shaped.runs {
            ...
            for glyph in &run.glyphs {
                ...
                if !Bounds::new(glyph_origin, glyph_bounds)
    ...
    Document {
        ...
        paragraphs: vec![
            Paragraph {
                ...
                runs: vec![plain(
                    "This is a Pocket. It’s the highest-level heading in the document.",
                )],
            },
            Paragraph {
                style: ParagraphStyle::Hat,
                runs: vec![plain(
                    "This is a Hat. It’s the second-highest level heading.",
                )],
            },
            Paragraph {
            ...
            Paragraph {
                ...
                runs: vec![
                    ...
                    run(
                        ...
                        RunStyles::default().with(RunStyle::HighlightSpoken),
                    ),
                    plain(", "),
                    run(
                        "underlined words",
                        RunStyles::default().with(RunStyle::Underline),
                    ),
                    plain(", and "),
                    run("emphasis", RunStyles::default().with(RunStyle::Emphasis)),
                    plain(
                        " while preserving the document as paragraph styles plus named run styles.",
                    ),
                ],
            },
            Paragraph {
            ...
            Paragraph {
                ...
                runs: vec![plain(
                    "This is a Block. It’s the third-highest level heading.",
                )],
            },
            Paragraph {
                ...
                runs: vec![
                    plain(
                        "This is a tag. It’s the fourth-highest level heading, tied with analytic.",
                    ),
            ...
            Paragraph {
                ...
                runs: vec![
                    ...
                    run("underlined", RunStyles::default().with_direct_underline()),
                    plain(
                        ", though it is uncommon, and in theory the cite text can be underlined too.)",
                    ),
                ],
            },
            Paragraph {
            ...
            Paragraph {
                style: ParagraphStyle::Normal,
                runs: vec![plain(
                    "It might continue for multiple paragraphs like this.",
                )],
            },
            Paragraph {
                style: ParagraphStyle::Normal,
                runs: vec![
                    ...
                    plain(
                        ", or sometimes the next other header if the evidence was the end of a section.",
                    ),
                ],
            },
            Paragraph {
                ...
                runs: vec![
                    ...
                    plain("It’"),
                    plain(
                        "s meant to be independent of evidence and deleted on the version of the document you send to your opponents. It can be ",
                    ),
                    run("underlined", RunStyles::default().with_direct_underline()),
                    plain("."),
                ],
